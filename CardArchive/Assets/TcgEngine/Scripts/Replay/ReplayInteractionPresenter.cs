...
namespace TcgEngine.Replay
{
    public class ReplayInteractionPresenter : MonoBehaviour
    {
        ...
        public IEnumerator Present(ReplayInteraction action)
        {
            ...
            {
                foreach (var card in FindObjectsByType<CardMulligan>().OrderBy(c => c.transform.GetSiblingIndex()))
                {
            ...
            if (action.kind == "card")
            {
                var card = FindObjectsByType<CardSelectorCard>().FirstOrDefault(c => c.GetCard().uid == action.target);
